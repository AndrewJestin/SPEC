...
function formatReviewTime(value, fallback) {
  const publishedAt = value ? new Date(value) : null;
  if (publishedAt && !Number.isNaN(publishedAt.getTime())) {
    const elapsedMs = Math.max(0, Date.now() - publishedAt.getTime());
  ...
  const time = String(fallback || "").trim();
...
function renderGoogleReviews(data) {
  ...
  data.reviews.slice(0, 5).forEach((review) => {
    ...
    meta.textContent = formatReviewTime(review.publishTime, review.relativeTime);
